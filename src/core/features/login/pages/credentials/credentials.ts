...
export default class CoreLoginCredentialsPage implements OnInit, OnDestroy {
    ...
    showLoginForm = true;
    platformName = 'UCG ONE';
    platformCategory = 'Grado';
    platformIcon = 'ucg-graduation-cap';
    ...
    async ngOnInit(): Promise<void> {
        try {
            ...
            const isPostgraduate = siteUrl.toLowerCase().includes('posgrado');

            this.platformName = isPostgraduate ? 'Posgrados' : 'UCG ONE';
            this.platformCategory = isPostgraduate ? 'Posgrado' : 'Grado';
            this.platformIcon = isPostgraduate ? 'ucg-book-open-01' : 'ucg-graduation-cap';
